...
| Operations/rollback | PRD §18-20 | RUNBOOK | Sprint 8 | install/upgrade/rollback rehearsal (`internal/app/upgrade_test.go`: pre-upgrade backup, additive schema upgrade preserving records, older-binary rejection, and rollback to the pre-upgrade backup); operator runbook verification (`cmd/routeweft/runbook_test.go`: migrate/backup/check-only restore/version/help CLI surface plus liveness, readiness, and session-gated admin/inference endpoints) |
